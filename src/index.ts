...
function setupApiRoutes() {
    ...
    // ── Memory / Session (for settings page) ──────────────────
    ...
    // ── AI Chat (for agent.html Mini App) ──────────────────────
    app.post('/api/chat', async (req, res) => {
        try {
            ...
            // Skip injected [SYSTEM] prefix, keep last 18 messages for context
            const history = messages
                .filter((m: any) => !m.content.startsWith('[SYSTEM]:'))
                .slice(-18);

            // deep_search mode: prefix so executor routes to Perplexer
...
async function bootstrap() {
    ...
    initByokDb(db); // wire BYOK to DB instance
    ...
    httpServer.listen(CONFIG.PORT, '0.0.0.0', () => {
        Logger.success('system', `NEXUM is LIVE → port ${CONFIG.PORT} 🌍`);
    });
}
